...
data "aws_iam_policy_document" "cloudwatch_reports_sns_encryption_key_policy_doc" {
  statement {
    sid     = "EnableRootAccessAndPreventPermissionDelegationForS3BucketSNSKMSKey"
    effect  = "Allow"
    actions = ["kms:*"]
    #checkov:skip=CKV_AWS_356:Without this statement, KMS key cannot be managed by root
    resources = ["${aws_kms_key.cloudwatch_reports_sns_encryption_key.arn}"]
    principals {
      type        = "AWS"
      identifiers = ["arn:aws:iam::${local.account_id}:root"]
    }
  ...
  statement {
    ...
    condition {
      test     = "ArnLike"
      variable = "AWS:SourceArn"
      values   = [aws_sns_topic.cloudwatch_reports_notifications.arn]
    }
  }
  statement {
    sid    = "AllowUseOfTheKeyForS3BucketSNSKMSKey"
