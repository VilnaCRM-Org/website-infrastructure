...
resource "aws_iam_policy" "terraform_ci_cd_policy" {
  ...
  tags        = var.tags
  policy      = data.aws_iam_policy_document.terraform_ci_cd_policy_document.json
}

resource "aws_iam_policy" "terraform_iam_policy" {
  count = local.is_ci_cd_infra ? 1 : 0

  name        = "${var.project_name}-terraform-role-iam-policy"
  description = "Policy to allow actions on IAM"
  tags        = var.tags
  policy      = data.aws_iam_policy_document.terraform_iam_policy_document.json
}