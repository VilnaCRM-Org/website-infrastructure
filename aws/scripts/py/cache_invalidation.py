...
def get_cloudfront_distributions() -> tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    try:
        ...
        raise RuntimeError(f"Failed to list CloudFront distributions: {error_code} - {error_message}")
    ...
    for dist in distributions:
        for origin in dist.get('Origins', {}).get('Items', []):
            ...
            if origin_domain == staging_origin:
                staging_dist = dist
                print(f"Found staging distribution: {dist['Id']} (origin: {origin_domain})")
                break
            elif origin_domain == production_origin:
                production_dist = dist
                print(f"Found production distribution: {dist['Id']} (origin: {origin_domain})")
                break
...
def invalidate_cache(distribution: Dict[str, Any], *, is_staging: bool = True) -> Dict[str, Any]:
    ...
    try:
        ...
        error_code = e.response['Error']['Code']
        error_message = e.response['Error']['Message']
        raise RuntimeError(f"Failed to create invalidation for {env}: {error_code} - {error_message}")

def wait_for_invalidation(distribution: Dict[str, Any], invalidation_id: str, *, timeout: int = 900) -> None:
    ...
    while time.time() - start < timeout:
        try:
            ...
            error_code = e.response['Error']['Code']
            error_message = e.response['Error']['Message']
            raise RuntimeError(f"Failed to check invalidation status: {error_code} - {error_message}")
    
    raise TimeoutError(f"Invalidation {invalidation_id} did not complete within {timeout}s")
