...
def type_handler(config_type, staging_dns_name):
    ...
    if config_type != "SingleHeader":
        return create_config(
            staging_dns_name, CLOUDFRONT_HEADER, config_type='header'
        )
    return create_config(
        staging_dns_name, CLOUDFRONT_WEIGHT, config_type='weight'
    )
...
def update_continuous_deployment_policy(policy_id, policy_etag, config_filename):
    ...
    subprocess.check_output(
        [
            ...
            f"file://{config_filename}", '--region', CLOUDFRONT_REGION,
