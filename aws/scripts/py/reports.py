        ...
        case _:
            ...
            return (
                ...
                generate_report(
                    ...
                    [
                        generate_report_link(
                            bucket_name, region, build_id, reports_dir
                        )
                    ]
                )
            )
...
def process_test(run, config, repository_dir, region, build_id):
    ...
    return run_handler(
        run, test_name, f"{repository_dir}/{reports_dir}",
        region, bucket_name, build_id, reports_dir
    )
...
def main():
    ...
    test_configs = {
        "LHCI_DESKTOP_RUN": (
            "Lighthouse Desktop Test", "lhci-reports-desktop",
            env_variables["lhci_reports_bucket_name"]
        ),
        "LHCI_MOBILE_RUN": (
            "Lighthouse Mobile Test", "lhci-reports-mobile",
            env_variables["lhci_reports_bucket_name"]
        ),
        "PW_E2E_RUN": (
            "Playwright E2E Test", "playwright-e2e-reports",
            env_variables["test_reports_bucket_name"]
        ),
        "PW_VISUAL_RUN": (
            "Playwright Visual Test", "playwright-visual-reports",
            env_variables["test_reports_bucket_name"]
        ),
        "MUTATION_RUN": (
            "Mutation Test", "reports/mutation",
            env_variables["test_reports_bucket_name"]
        ),
        "UNIT_RUN": (
            "Unit Test", "coverage/lcov-report",
            env_variables["test_reports_bucket_name"]
        ),
        "LOAD_RUN": (
            "Load Test", "src/test/load/results",
            env_variables["test_reports_bucket_name"]
        ),
    ...
    for key, config in test_configs.items():
        if key in os.environ:
            test_result, report = process_test(
                key, config, repository_dir,
                env_variables["region"], env_variables["build_id"]
            )
